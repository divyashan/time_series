...
from utils import evaluate_test_embedding, standardize_ts_lengths, UCR_DATASETS, MV_DATASETS
...
def test_method(dataset, pool_pctg, layer_size_1, layer_size_2):
  ...
  if dataset in UCR_DATASETS:
    ...
    X_train = dataset[0].X_train
    y_train = dataset[0].y_train
    X_test = dataset[0].X_test
    y_test = dataset[0].y_test
  ...
  with tf.name_scope("Conv2") as scope:
    W_conv2 = tf.get_variable("Conv_Layer_2", shape=[5, 1, num_filt_1, num_filt_2],initializer=initializer)
  ...
  with tf.name_scope('Batch_norm_conv2') as scope:
      ...
      h_conv2 = max_pool_2x2(h_conv2, pool_width)
  """
  ...
  with tf.Session() as sess:
    ...
    for i in range(max_iterations):
      ...
      if i%50 == 0:
        ...
        step +=1
      gg = h_relu.eval(feed_dict={x:X_train[batch_ind], y_: y_train[batch_ind], keep_prob: dropout, bn_train : True})
...
test_method(sys.argv[1], .1, 40, 40)
#plt.show()
