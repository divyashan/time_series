...
with tf.variable_scope("siamese") as scope:
    model1, filters= build_conv_net(anchor,bn_train,dropout_f, ts_length, embedding_size, pool_width)
    scope.reuse_variables()
    model2, _ = build_conv_net(same,bn_train,dropout_f, ts_length, embedding_size, pool_width)
    scope.reuse_variables()
    model3, _ = build_conv_net(different,bn_train,dropout_f, ts_length, embedding_size, pool_width)
...
with tf.Session() as sess:
    ...
    for epoch in range(400):
        ...
        for i in range(total_batch):
            ...
            input1,input2,input3 =next_batch(s,e,tr_trips)
        ...
        if epoch % 50  == 0:
            tr_acc = compute_accuracy(tr_trips)
