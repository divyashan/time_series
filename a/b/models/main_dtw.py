...
def gen_pairwise_dtw_dict(tr_pairs):
    dtw_list = []
    for i,pair in enumerate(tr_pairs):
        d , _ = fastdtw(pair[0],pair[1])
        dtw_list.append(d)
    return dtw_list

def gen_pairwise_dtw_arr(X):
    n_samples = X.shape[0]
    dtw_arr = np.zeros((n_samples, n_samples))
...
def dtw_loss(ts_L, ts_R, model1, model2):
    dtw_dists = []
    n_samples = ts_L.shape[0]
    indices = range(batch_size)
    for ts_i in indices:
        ts1, ts2 = ts_L[ts_i], ts_R[ts_i]
        dist , _ = fastdtw(ts1, ts2)
        dtw_dists.append(dist)
    dtw_dists = np.array(dtw_dists)

    euclid_dists = tf.norm(tf.subtract(model1, model2), axis=1)
    # Could be squared distance? What's the point of that?
    return tf.subtract(euclid_dists, dtw_dists)
...
dtw_list = gen_pairwise_dtw_dict(tr_pairs)
...
loss = dtw_loss(images_L, images_R, model1, model2)
ugh = c_loss(labels, model1, model2)
dtw_loss = get_dtw_loss(model1, model2, ind)
